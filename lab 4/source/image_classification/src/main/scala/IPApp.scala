...
object IPApp {
  ...
  val IMAGE_CATEGORIES = List("rice",  "bibimap", "sushi", "sausage", "oden", "omelet", "jiaozi")
  ...
  def extractDescriptors(sc: SparkContext, images: RDD[(String, String)]): Unit = {
    ...
    val data = images.map {
      case (name, contents) => {
        val desc = ImageUtils.descriptors(name.split("file:/")(1))//sift
...
featuresSeq.take(1).foreach(println)
